...
export function DriverComparison(){
    ...
    useEffect(() => {
        const loadDrivers = async () => {
            ...
            const sortedDrivers = [...drivers].sort(compare);
            setDriversList(sortedDrivers);
            
            if (urlDriver1) {
                const d1 = drivers.find(driver => driver.id === urlDriver1);
                if (d1) {
                    setURLDriverName1(d1.name);
                    setInputDriver1({ value: urlDriver1, label: d1.name });
                }
            }
            if (urlDriver2) {
                const d2 = drivers.find(driver => driver.id === urlDriver2);
                if (d2) {
                    setURLDriverName2(d2.name);
                    setInputDriver2({ value: urlDriver2, label: d2.name });
                }
            }
        ...
        loadDrivers();
    }, [urlDriver1, urlDriver2, compare]);
    
    useEffect(() => {
        if (urlDriverName1 && inputdriver1.value === urlDriver1 && inputdriver1.label !== urlDriverName1) {
            setInputDriver1({ value: urlDriver1, label: urlDriverName1 });
        }
    }, [urlDriver1, urlDriverName1, inputdriver1.value, inputdriver1.label]);

    useEffect(() => {
        if (urlDriverName2 && inputdriver2.value === urlDriver2 && inputdriver2.label !== urlDriverName2) {
