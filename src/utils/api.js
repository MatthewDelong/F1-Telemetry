...
export function normalizeOpenF1Date(date) {
  ...
  return (
    d.toISOString().split(".")[0] +
    "." +
    d.toISOString().split(".")[1].slice(0, 3)
  );
}
...
export const fetchDriverStats = async (driverId1, driverId2) => {
  const fetchDriverData = async (driverId) => {
    try {
      const dataResponse1 = await fetch(
        `${BASE_F1_URL}drivers/${driverId}.json`,
      );
      if (dataResponse1.ok) {
  ...
  try {
    ...
    if (Number(selectedYear) === 2026) {
      filteredRacesData = filteredRacesData.filter(
        (race) => !CANCELLED_RACES_2026.includes(race.meeting_name),
      );
    }

    // Fetch sessions (using cached sessionsData if possible, but specifically for 'Race' filter)
    const f1apiMeetingSessionsList = await fetchWithPersistentCache(
      `${buildOpenF1Url("/sessions")}?year=${selectedYear}&session_name=Race`,
    );

    // Filter races based on meeting_key presence in sessions
    const filteredRaces = filteredRacesData.filter(
      (race) =>
        Array.isArray(f1apiMeetingSessionsList) &&
        f1apiMeetingSessionsList.some(
    ...
    if (response.ok) {
      ...
      const raceResultsPromises = races.map((race, index) => {
        if (new Date(race.date) < new Date()) {
          return fetchRaceResults(selectedYear, race.round).then((results) => ({
            ...race,
            results,
          }));
        } else {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const results = data.map((result) => ({
        ...
        interval: result.Time?.time || null,
...
export const fetchRaceResultsByCircuit = async (year, circuitId) => {
  try {
    ...
    const results = data.find(
      (element) => element.Circuit.circuitId === circuitId,
    ).Results;
    return results || [];
  } catch (error) {
    console.error("Error fetching race results:", error);
