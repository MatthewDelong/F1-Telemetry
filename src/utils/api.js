...
export const fetchOpenF1Data = async (url, retries = 7, backoff = 1500) => {
  try {
    ...
    if (response.status === 429 && retries > 0) {
      console.warn(
        `[API] Rate limited (429) on ${url}. Retrying in ${backoff}ms... (${retries} retries left)`,
      );
      await delay(backoff);
    ...
    if (!response.ok) {
      ...
      if (response.status === 401 || response.status === 403) {
        console.warn(
          `[API] Unauthorized or Forbidden (${response.status}) on ${url}. API is likely restricted due to a live session. Returning empty array.`,
        );
    ...
    if (retries > 0) {
      console.warn(
        `[API] Fetch error for ${url}. Retrying in ${backoff}ms... (${retries} retries left)`,
      );
      await delay(backoff);
      return fetchOpenF1Data(url, retries - 1, backoff * 2);
    }
 ...
 */
export const fetchOpenF1FullSessionData = async (
  endpoint,
  sessionKey,
  extraParams = "",
) => {
  ...
  try {
    ...
    if (cached) {
      ...
      if (Date.now() - timestamp < CACHE_TTL && data && data.length > 0) {
        console.log(
          `[API] Loaded full session data from cache for ${endpoint}`,
        );
        return data;
      }
    }
    ...
    console.warn(
      "[Cache] Failed to read localStorage cache for full session data:",
      e,
    );
  }
  ...
  try {
    while (hasMore) {
      ...
      if (extraParams) {
        const search = extraParams.startsWith("?")
          ? extraParams.slice(1)
          : extraParams;
        url += `&${search}`;
      }
      ...
      if (lastDate) {
        const filterName =
          endpoint.includes("laps") || endpoint.includes("sessions")
            ? "date_start"
            : "date";
      ...
      if (!batch || batch.length === 0) {
        ...
        if (allData.length === 0) {
          console.log(
            `[API] ${endpoint} Keys in first record:`,
            Object.keys(firstItem),
          );
        }

        const firstDate = normalizeOpenF1Date(
          firstItem.date || firstItem.date_start,
        );
        const nextLastDate = normalizeOpenF1Date(
          lastItem.date || lastItem.date_start,
        );

        console.log(
          `[API] ${endpoint} Batch: ${batch.length} records. Range: [${firstDate}] -> [${nextLastDate}]`,
        );
    ...
    if (allData.length > 0) {
      try {
        ...
        console.warn(
          "[API] Failed to cache full session data (might be too large)",
        );
      }
    }
...
export const fetchWithPersistentCache = async (url) => {
  ...
  try {
    ...
    if (url.includes("source=f1") || url.includes("/proxy/f1/")) {
      try {
        let cleanPath = url.includes("path=")
          ? url.split("path=")[1]
          : url.includes("/proxy/f1/")
            ? url.split("/proxy/f1/")[1]
            : "";
            ...
            try {
              ...
              if (ghResp.ok) {
                ...
                if (
                  ghData &&
                  !ghData.error &&
                  !(Array.isArray(ghData) && ghData.length <= 1)
                ) {
                  console.log(
                    `[Cache Fallback] Loaded ${cleanPath} from raw GitHub`,
                  );
                  return ghData;
                }
              }
    ...
    if (cachedData) {
      console.warn(
        `[Cache] Network failed for ${url}, returning stale data.`,
        error,
      );
      return cachedData;
    }
...
export const fetchDriversList = async () => {
  ...
  try {
    const response = await fetchWithPersistentCache(
      `${BASE_F1_URL}driversList.json?v=2026_v3`,
    );
    if (
    ...
    console.warn(
      "[API] fetchDriversList response was not a valid driver array. Triggering static/GitHub fallback...",
    );
  } catch (e) {
...
export const fetchDriverStats = async (
  ...
  const fetchDriverData = async (driverId) => {
    ...
    try {
      ...
      console.warn(
        `[API] Primary fetch failed for driver ${driverId} (HTTP ${dataResponse1?.status}). Trying raw GitHub fallback...`,
      );
    } catch (error) {
      console.warn(
        `[API] Error fetching driver data for ${driverId} via primary endpoint. Trying raw GitHub fallback...`,
        error,
      );
    }
    ...
    try {
      ...
      console.error(
        `[API] Raw GitHub fallback failed for driver ${driverId}: HTTP ${ghResponse.status}`,
      );
    } catch (ghErr) {
      console.error(
        `[API] Error fetching raw GitHub fallback for driver ${driverId}:`,
        ghErr,
      );
    }
...
export const fetchRaceMeetingKeys = async (selectedYear) => {
  ...
  try {
    const raceResponse = await fetch(
      `${BASE_F1_URL}races/races.json?t=${Date.now()}`,
    );
    if (!raceResponse.ok) {
...
export const fetchRacesAndSessions = async (selectedYear) => {
  try {
    let filteredRacesData = [];

    // PRIORITY 1: Fetch from our local/GitHub races.json first (fast, avoids OpenF1 rate limits, uses api_cache/f1 on Ionos)
    console.log("[API] Prioritizing local races.json for meetings...");
    const meetingKeysObj = await fetchRaceMeetingKeys(selectedYear);

    if (meetingKeysObj && Object.keys(meetingKeysObj).length > 0) {
      filteredRacesData = Object.entries(meetingKeysObj).map(
        ([name, data]) => ({
          meeting_name: name,
      ...
      // PRIORITY 2: Fallback to OpenF1 if races.json is empty or missing
      console.warn(
        "[API] Local races.json empty, falling back to OpenF1 /meetings...",
      );
      const racesData = await fetchWithPersistentCache(
        `${buildOpenF1Url("/meetings")}?year=${selectedYear}`,
      );
      filteredRacesData = Array.isArray(racesData) ? racesData : [];
    }

    // Filter out cancelled races for 2026
    if (Number(selectedYear) === 2026) {
      filteredRacesData = filteredRacesData.filter(
        (race) =>
    ...
    // Note: We no longer fetch /sessions from OpenF1 to filter the meetings.
    // races.json only contains valid races, so we don't need to cross-reference with /sessions anymore, saving an OpenF1 request!

    return filteredRacesData;
  } catch (error) {
    console.error("Error fetching data:", error);
    return [];
  }
    ...
    if (response.ok) {
      ...
      if (Number(selectedYear) === 2026) {
        races = races.filter((race) => {
          ...
          return !CANCELLED_RACES_2026.some((c) =>
            name.includes(c.toLowerCase().replace(" grand prix", "")),
          );
        });
      }
  ...
  try {
    ...
    if (rawData && Array.isArray(rawData)) {
      ...
      if (!raceData && fallbackRaceName) {
        const searchName = fallbackRaceName
          .toLowerCase()
          .replace(" grand prix", "");
      ...
      console.log(
        `[API Debug] Results for ${selectedYear} Round ${raceId}:`,
        data.length,
        "results",
      );
      const results = data.map((result) => {
        const drv =
          result.Driver ||
          (result.driver && typeof result.driver !== "function"
            ? result.driver
            : undefined) ||
          {};
        const con =
          result.Constructor ||
          (result.constructor && typeof result.constructor !== "function"
            ? result.constructor
            : undefined) ||
          {};
        return {
          driver: {
            ...drv,
            nationality:
              drv?.nationality || drv?.country_code || drv?.country || "",
          },
      ...
      try {
        ...
        console.warn(
          "[Cache] Failed to write race results to localStorage:",
          e,
        );
      }
...
export const getPartialConstructorStandings = async (
  selectedYear,
  start,
  end,
) => {
  ...
  try {
    ...
    if (!Array.isArray(endData)) {
      console.warn(
        `[API] No constructor standings data found for round ${end}`,
      );
      return [];
    }
    ...
    const constructorStandings = startStandings.map((start) => {
      const end = endStandings.find(
        (end) => end.constructorId === start.constructorId,
      );
      return {
    ...
    driverStandings.forEach((standing) => {
      standing.Constructors.forEach((constructor) => {
        const constructorIndex = constructorStandings.findIndex(
          (c) => c.constructorId === constructor.constructorId,
        );
        if (constructorIndex !== -1) {
          constructorStandings[constructorIndex].driverCodes.push(
    ...
    return constructorStandings.sort(
...
export const getConstructorStandings = async (selectedYear) => {
  ...
  try {
    ...
    const constructorStandings = data_constructor.map((standing) => ({
      ...
      points: standing.points,
      driverCodes: [],
      constructorColor: teamColors[selectedYear]?.[
    ...
    const driverFetchPromises = constructorStandings.map(
      async (constructorStanding) => {
        ...
        if (driverResponse.ok) {
          ...
          constructorStanding.driverCodes = drivers.map(
            (driver) => driver.code,
          );
        } else {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const standings = endStandings.map((end) => {
        const start = startStandings.find(
          (start) => start.Driver.driverId === end.Driver.driverId,
        );
        return {
          ...
          constructorColor: teamColors[selectedYear]?.[
            end.Constructors[0].constructorId
          ]
...
export const fetchDriversAndTires = async (sessionKey) => {
  ...
  try {
    ...
    const stintsByDriver = stintsData.reduce(
      (acc, { driver_number, lap_end, compound }) => {
...
export const fetchRaceResultsByCircuit = async (
  year,
  circuitId,
  raceName = "",
) => {
  try {
    ...
    const raceData = data.find((element) => {
      ...
      const matchId =
        element.Circuit && element.Circuit.circuitId === circuitId;
      const matchName =
        raceName &&
        element.raceName &&
      ...
      const matchLocality =
        circuitId &&
        element.Circuit?.Location?.locality &&
        element.Circuit.Location.locality.toLowerCase() ===
          circuitId.toLowerCase();
...
export const fetchQualifyingResultsByCircuit = async (
  year,
  circuitId,
  raceName = "",
) => {
...
export const fetchSprintResultsByCircuit = async (
  year,
  circuitId,
  raceName = "",
) => {
...
export async function fetchTrackReferenceData(sessionKey, circuitId = null) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data && data.length > 0) {
          console.log(
            `[API] Track reference loaded from local static file for ${canonicalId}`,
          );
          return data;
        }
      }
  ...
  try {
    ...
    if (cached) {
      ...
      if (Date.now() - timestamp < CACHE_TTL && data && data.length > 0) {
        console.log(
          `[API] Track reference loaded from cache (${data.length} points)`,
        );
        return data;
      }
    }
    ...
    console.warn(
      "[Cache] Failed to read track reference from localStorage:",
      e,
    );
  }
  ...
  try {
    ...
    if (!lapsData || lapsData.length === 0) {
      console.log(
        `[API] No laps for session ${sessionKey}. Attempting 2024 fallback...`,
      );
      try {
        const sessionInfo = await fetchWithPersistentCache(
          `${buildOpenF1Url("/sessions")}?session_key=${sessionKey}`,
        );
        if (sessionInfo && sessionInfo.length > 0) {
          const circuitKey = sessionInfo[0].circuit_key;
          const pastSessions = await fetchWithPersistentCache(
            `${buildOpenF1Url("/sessions")}?circuit_key=${circuitKey}&year=2024`,
          );
          const pastRace = pastSessions.find((s) => s.session_name === "Race");
          if (pastRace && pastRace.session_key !== sessionKey) {
            console.log(
              `[API] Found 2024 fallback session ${pastRace.session_key} for circuit ${circuitKey}`,
            );
    ...
    const validLaps = lapsData.filter(
      (l) =>
        l.lap_duration > 50 &&
    ...
    console.log(
      `[API] Fetching 1 precise lap for track ref: Driver ${driver_number} from ${date_start} to ${date_end}`,
    );
    ...
    while (hasMore && safetyCounter < 10) {
      ...
      const newItems =
        rawLocationData.length > 0 &&
        batch[0].date === rawLocationData[rawLocationData.length - 1].date
          ? batch.slice(1)
          : batch;
    ...
    const locationData = rawLocationData.filter(
      (p) => new Date(p.date).getTime() <= endMs,
    );
    ...
    console.log(
      `[API] Track reference: ${trackPoints.length} points for exact lap`,
    );
    ...
    try {
      ...
      console.warn(
        "[Cache] Failed to write track reference to localStorage:",
        e,
      );
    }
...
export async function fetchLocationData(
  ...
  console.log("[DEBUG] Fetching location/car data with normalized dates:", {
    normStartTime,
    normEndTime,
  });
...
export const fetchOpenF1Podium = async (meetingKey) => {
  ...
  try {
    ...
    if (
      ...
      console.warn(
        `[OpenF1 Fallback] No session found for meeting ${meetingKey}`,
      );
      return [];
    }
    ...
    await delay(150);
    let driversData = await fetchWithPersistentCache(
      `${buildOpenF1Url("/drivers")}?session_key=${sk}`,
    );
    // If cached driversData is empty, try fetching fresh data to avoid 6-hour empty cache
    if (!driversData || driversData.length === 0) {
      driversData = await fetchOpenF1Data(
        `${buildOpenF1Url("/drivers")}?session_key=${sk}`,
      );
    }
    await delay(150);
    const intData = await fetchOpenF1Data(
      `${buildOpenF1Url("/intervals")}?session_key=${sk}`,
    );
    await delay(150);
    ...
    posData.forEach(
      (p) => (driverPositions[p.driver_number] = parseInt(p.position, 10)),
    );

    const driverLaps = {};
    lapsData.forEach((l) => {
      if (
        !driverLaps[l.driver_number] ||
        l.lap_number > driverLaps[l.driver_number]
      ) {
    ...
    lapsData.forEach((l) => {
      if (l.lap_duration) {
        if (
          !driverBestLaps[l.driver_number] ||
          l.lap_duration < driverBestLaps[l.driver_number]
        ) {
    ...
    const results = top3Drivers.map((td) => {
      const drv =
        driversData.find(
          (d) => parseInt(d.driver_number, 10) === td.driver_number,
        ) || {};
      ...
      const fastestLapData =
        lapsData.find(
          (l) =>
            parseInt(l.driver_number, 10) === td.driver_number &&
            l.lap_duration === bestLapSec,
        ) || {};
      ...
      return {
        ...
        Driver: {
          code:
            drv.name_acronym ||
            drv.last_name?.substring(0, 3).toUpperCase() ||
            `NO${td.driver_number}`,
          familyName: drv.last_name,
          nationality: drv.country_code,
        },
        Constructor: {
          constructorId: (drv.team_name || "")
            .toLowerCase()
            .replace(/\s+/g, "_"),
...
export const fetchMostRecentRace = async (
  selectedYear,
  specificRound = null,
  specificRaceName = null,
) => {
  ...
  try {
    ...
    console.warn(
      "[Cache] Failed to read most recent race from localStorage:",
      e,
    );
  }
  ...
  try {
    ...
    let raceDetails = await fetchWithPersistentCache(
      `${BASE_F1_URL}races/${selectedYear}/raceDetails.json`,
    );

    // Filter out cancelled races for 2026
    if (Number(selectedYear) === 2026) {
      raceDetails = raceDetails.filter(
        (race) => !CANCELLED_RACES_2026.includes(race.raceName),
      );
    }
    ...
    if (specificRaceName) {
      ...
      mostRecentRace = raceDetails.find(
        (r) => parseInt(r.round, 10) === specificRound,
      );
    } else {
      // Filter out races that haven't happened yet
      const pastRaces = raceDetails.filter(
    ...
    const meetingKey =
      meetingKeys[mostRecentRace.raceName]?.meeting_key || "unknown";

    // Fetch race results for the most recent race
    let raceResults = await fetchRaceResults(
      selectedYear,
      mostRecentRace.round,
    ...
    if (
      ...
      console.log(
        `[API] Results empty for ${mostRecentRace.raceName}, falling back to OpenF1...`,
      );
      ...
      const hasFastestLap = raceResults.some(
        (r) => r.fastestLap?.rank === "1" || r.FastestLap?.rank === "1",
      );
      if (!hasFastestLap) {
        try {
          console.log(
            `[API] FastestLap missing for ${mostRecentRace.raceName}, augmenting from OpenF1...`,
          );
          const oF1Results = await fetchOpenF1Podium(meetingKey);
          if (oF1Results && oF1Results.length > 0) {
            raceResults = raceResults.map((r) => {
              const of1Driver = oF1Results.find(
                (o) => parseInt(o.position, 10) === parseInt(r.position, 10),
              );
    ...
    try {
      if (raceResults && raceResults.length > 0) {
        localStorage.setItem(
          cacheKey,
          JSON.stringify({ data: raceWithDetails, timestamp: Date.now() }),
        );
      }
    } catch (e) {
      console.warn(
        "[Cache] Failed to write most recent race to localStorage:",
        e,
      );
    }
