...
export function TeammateComparisonButton({ year = DEFAULT_YEAR, className }) {
  ...
  const pickRandomTeam = (currentConstructorId) => {
    ...
    let randomTeam =
      TEAMMATE_TEAMS[Math.floor(Math.random() * TEAMMATE_TEAMS.length)];
  ...
  return (
    ...
    >
      ...
      >
        <img
          src={`/images/${year}/cars/${activeTeam.constructorId}.png`}
        ...
        {activeTeam.drivers.map((driverCode) => (
          <img
            key={driverCode}
            src={`/images/${year}/drivers/${driverCode}.png`}
      ...
      <button
        ...
        className={classNames(
          "rounded px-24 py-8 text-white transition-all duration-200 font-display max-sm:bg-brand-blue-500 bg-glow-dark",
        )}
        style={
          isHovered
            ? {
                boxShadow: `inset 0 0 2.4rem 0 rgba(0, 0, 0, .75), 0 0 2.4rem 0 #${activeTeamColor}`,
              }
            : undefined
        }
