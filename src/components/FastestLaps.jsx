      ...
      <div className="bg-glow-large h-fit p-32 mb-16 rounded-md sm:rounded-xlarge">
        ...
        <ul>
          {raceResults
            ...
            .sort(
              (a, b) =>
                parseInt(a.FastestLap.rank) - parseInt(b.FastestLap.rank),
            )
            .map((result, index) => (
        ...
        {(!raceResults ||
          raceResults.filter((r) => r.FastestLap && r.FastestLap.rank)
            .length === 0) && (
          <div className="text-center text-neutral-500 py-16">
            Fastest lap data will be available once the session results are
            finalized.
          </div>
        )}
      </div>
